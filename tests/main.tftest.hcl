...
run "validate_ssh_cidr_blocks" {
  ...
  assert {
    condition = alltrue([
      for rule in aws_security_group.debug_sg.ingress :
        contains(rule.cidr_blocks, "10.0.0.0/8") && contains(rule.cidr_blocks, "172.16.0.0/12")
    ])
    error_message = "Security group should include custom CIDR blocks"
  }
}
